...
func (r *Root) initTraceroute(measurementFlags *pflag.FlagSet, localFlags *pflag.FlagSet) {
	var tracerouteCmd = &cobra.Command{
		...
		Use:     "traceroute [target1[, target2]] from [location | measurement ID | @1 | first | @-1 | last | previous]",
...
Examples:
  ...
  traceroute jsdelivr.com from europe+eyeball-network --share`,
	...
	localFlags.String("protocol", "ICMP", "specify the protocol to use for tracerouting: ICMP, TCP, or UDP")
	localFlags.Uint16("port", 80, "specify the port to use for the traceroute; only applicable for the TCP protocol")
